...
bool ListNode::swapWithNext() {
    if (!isEmpty()) {
        if (cursor->next) {
            // create int to hold data of cursor
            int cursorData = cursor->data;
            // swap from next to cursor
            cursor->data = cursor->next->data;
            // and from old cursor data to next
            cursor->next->data = cursorData;
            // then move cursor
            cursor = cursor->next;
            return true; // did swap
        } else {
